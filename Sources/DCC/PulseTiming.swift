...
///
/// `PulseTiming` is initialized with a `pulseWidth` giving the duration in microseconds of the
...
public struct PulseTiming {
    ...
    public enum Error : Swift.Error {
        ...
        /// Thown by PulseTiming.init when it cannot produce standards conformant timings
    ...
    public init(pulseWidth: Float) throws {
        ...
        oneBitLength = Int((PulseTiming.oneBit - 1) / pulseWidth) + 1
        zeroBitLength = Int((PulseTiming.zeroBit - 1) / pulseWidth) + 1
        
        railComDelayLength = Int((PulseTiming.railComDelayMin - 1) / pulseWidth) + 1
        railComLength = Int((PulseTiming.railComMin - 1) / pulseWidth) + 1
        
        railComCount = (railComLength - 1) / (oneBitLength * 2) + 1
        
        preambleCount = PulseTiming.preambleCountMin + railComCount
        
        // Sanity check the lengths.
        guard ((Float(oneBitLength) * pulseWidth) >= PulseTiming.oneBitMin) &&
            ((Float(oneBitLength) * pulseWidth) <= PulseTiming.oneBitMax) else {
                throw Error.conformanceError(message: "Duration of one bit would be \(Float(oneBitLength) * pulseWidth)µs which is outside the valid range \(PulseTiming.oneBitMin)–\(PulseTiming.oneBitMax)µs")
        }
        guard ((Float(zeroBitLength) * pulseWidth) >= PulseTiming.zeroBitMin) &&
            ((Float(zeroBitLength) * pulseWidth) <= PulseTiming.zeroBitMax) else {
                throw Error.conformanceError(message: "Duration of zero bit would be \(Float(zeroBitLength) * pulseWidth)µs which is outside the valid range \(PulseTiming.zeroBitMin)–\(PulseTiming.zeroBitMax)µs")
        }
        guard ((Float(railComDelayLength) * pulseWidth) >= PulseTiming.railComDelayMin) &&
            ((Float(railComDelayLength) * pulseWidth) <= PulseTiming.railComDelayMax) else {
                throw Error.conformanceError(message: "Duration of pre-RailCom cutout delay would be \(Float(railComDelayLength) * pulseWidth)µs which is outside the valid range \(PulseTiming.railComDelayMin)–\(PulseTiming.railComDelayMax)µs")
        }
        guard ((Float(railComLength) * pulseWidth) >= PulseTiming.railComMin) &&
            ((Float(railComLength) * pulseWidth) <= PulseTiming.railComMax) else {
                throw Error.conformanceError(message: "Duration of RailCom cutout would be \(Float(railComLength) * pulseWidth)µs which is outside the valid range \(PulseTiming.railComMin)–\(PulseTiming.railComMax)µs")
        }
    }
