...
class PulseTimingTests: XCTestCase {
    ...
    func testTenMsOneBit() {
        let timing = try! PulseTiming(pulseWidth: 10)
    ...
    func testGoldilocksMsOneBit() {
        let timing = try! PulseTiming(pulseWidth: 14.5)
    ...
    func testFailOneBit() {
        XCTAssertThrowsError(try PulseTiming(pulseWidth: 25))
    }
    ...
    func testFailRailComDelay() {
        XCTAssertThrowsError(try PulseTiming(pulseWidth: 58))
    }
